...
root_path = root_path[0:root_path.find("/thalamocortical")+1]
...
class InterneuronTemplate(object):
    ...
    def createSynapse(self, cell, location, distal, spikes):
        ...
        if(distal==True):
            syn = LFPy.Synapse(cell, idx=0, **self.DISTAL_SYN) #location
        else:
            syn = LFPy.Synapse(cell, idx=0, **self.PROXIMAL_SYN) # location
